...
class PhiPhatSinhModel
{
    ...
    static async AddPhiPhatSinh(ci_name, ci_cost, ci_u_id, ci_stt_id)
    {
        return new Promise(resolve =>{
            db.query("INSERT INTO costs_incurred values(NULL, ?, ?, ?, ?)", [ci_name, ci_cost, ci_u_id, ci_stt_id], (err, result)=>{
                if(!err) resolve(true);
            })
        })
    }
